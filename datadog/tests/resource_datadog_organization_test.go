...
func TestAccDatadogOrganization_Create(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccCheckDatadogOrganizationConfigRequired(organizationName),
...
func TestAccDatadogOrganization_IncorrectName(t *testing.T) {
	t.Parallel()
	ctx, accProviders := testAccProviders(context.Background(), t)
...
func testAccCheckDatadogOrganizationConfigRequired(uniq string) string {
	return fmt.Sprintf(`
resource "datadog_organization" "foo" {
  name = "%s"
}`, uniq)
}
